...
class DeformCrossAttention_DFA3D(DeformCrossAttention):
    ...
    def __init__(self, embed_dims=256, num_cams=6, bev_h=200, bev_w=200, use_empty=False, num_head=8, pc_range=None, dropout=0.1, init_cfg=None, batch_first=False, deformable_attention=dict(type='MSDeformableAttention3D', embed_dims=256, num_levels=4), **kwargs):
        ...
        # RCF: reliability-conditioned fusion of the color and grayscale branches inside the
        # deformable cross-attention. Zero-init (bias=-4 => w~0) so training starts from the
        # color-only baseline and raises w only where the grayscale branch helps.
        self.use_rcf = kwargs.get('use_rcf', False)
        # Fix the reliability weight to a constant instead of learning it per voxel query.
        self.rcf_fixed_w = kwargs.get('rcf_fixed_w', None)
        # When True, stash the last per-query reliability w for visualization.
        self.rcf_dump_w = kwargs.get('rcf_dump_w', False)
        self._last_w = None
        if self.use_rcf and self.rcf_fixed_w is None:
            _hid = max(embed_dims // 2, 32)
            self.reliability = nn.Sequential(
                nn.Linear(embed_dims, _hid), nn.ReLU(inplace=True), nn.Linear(_hid, 1))
            nn.init.zeros_(self.reliability[-1].weight)
            nn.init.constant_(self.reliability[-1].bias, -4.0)
    ...
    def forward(self,
        ...
        _q_flat = queries_rebatch.view(bs*self.num_cams, max_len, self.embed_dims)
        queries, update_weight = self.deformable_attention(query=_q_flat, key=key, value=value,
        ...
        if self.use_rcf and value_gray is not None:
            # RCF: attend to the grayscale value with the same depth-aware deformable attention,
            # then combine per query with the reliability weight w.
            value_gray = value_gray.permute(2, 0, 1, 3).reshape(bs * self.num_cams, l, self.embed_dims)
            gray_queries, _ = self.deformable_attention(query=_q_flat, key=key, value=value_gray,
                                            value_dpt_dist=value_dpt_dist,
                                            reference_points=reference_points_rebatch.view(bs*self.num_cams, max_len, D, 3), spatial_shapes=spatial_shapes,
                                            level_start_index=level_start_index, **kwargs)
            if self.rcf_fixed_w is not None:
                w = torch.full((_q_flat.shape[0], _q_flat.shape[1], 1),
                               float(self.rcf_fixed_w), device=_q_flat.device, dtype=queries.dtype)
            else:
                w = torch.sigmoid(self.reliability(_q_flat))   # (bs*num_cams, max_len, 1)
            if self.rcf_dump_w:
                self._last_w = w.detach()
                # indexes[cam] lists the query positions visible to that camera; needed to map
                # w back to the voxel grid for visualization.
                self._last_indexes = [ix.detach().cpu() for ix in indexes]
            queries = (1.0 - w) * queries + w * gray_queries
