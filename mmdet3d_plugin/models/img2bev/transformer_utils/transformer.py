...
class PerceptionTransformer_DFA3D(PerceptionTransformer):
    ...
    def get_vox_features(
        ...
        # RCF: flatten grayscale features identically -> second value stream for the cross-attention
        value_gray_flatten = None
        if mlvl_feats_gray is not None:
            _gf = []
            for lvl, feat in enumerate(mlvl_feats_gray):
                feat = feat.flatten(3).permute(1, 0, 3, 2)
                if self.use_cams_embeds:
                    feat = feat + self.cams_embeds[:, None, None, :].to(feat.dtype)
                feat = feat + self.level_embeds[None, None, lvl:lvl + 1, :].to(feat.dtype)
                _gf.append(feat)
            value_gray_flatten = torch.cat(_gf, 2).permute(0, 2, 1, 3)
