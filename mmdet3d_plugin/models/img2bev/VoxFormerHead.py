...
class VoxFormerHead(nn.Module):
    ...
    def forward(self, mlvl_feats, proposal, cam_params, lss_volume=None, img_metas=None,  **kwargs):
        ...
        """
        mlvl_feats_gray = kwargs.pop('mlvl_feats_gray', None)  # RCF: gray value stream (cross-attn only)
        ...
        # Stash the unmasked-query set and coordinates so the reliability weight can be mapped to voxels for visualization.
        self._last_unmasked_idx = unmasked_idx.detach().cpu()
        self._last_vox_coords = vox_coords.detach().cpu()
