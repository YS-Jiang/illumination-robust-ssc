...
class GeometryDepth_Net(BaseModule):
    def __init__(
        ...
        # GSF: fuse the color stereo-depth volume with the grayscale stereo-depth volume through a
        # learned per-pixel reliability gate. Zero-init (bias=-4 => w~0) starts at color-only.
        self.use_gray_depth = use_gray_depth
        # Optionally fix the gate to a constant instead of learning it per pixel.
        self.geo_fixed_w = geo_fixed_w
        if use_gray_depth and geo_fixed_w is None:
            _h = max(self.D // 2, 16)
            self.geo_reliability = nn.Sequential(
                nn.Conv2d(2 * self.D, _h, 3, padding=1), nn.ReLU(inplace=True),
                nn.Conv2d(_h, 1, 1))
            nn.init.zeros_(self.geo_reliability[-1].weight)
            nn.init.constant_(self.geo_reliability[-1].bias, -4.0)
    ...
    def forward(self, input, img_metas):
        ...
        # GSF: fuse the grayscale stereo-depth volume via the per-pixel reliability gate.
        if self.use_gray_depth and img_metas.get('gray_stereo_depth', None) is not None:
            gray_depth = img_metas['gray_stereo_depth']
            _, gray_volume = self.get_downsampled_gt_depth(gray_depth)
            gray_volume = gray_volume.view(B, H, W, -1).permute(0, 3, 1, 2)
            gray_volume = self.stereo_volume_encoder(gray_volume)
            gray_volume = self.get_depth_dist(gray_volume)
            self._vis_dc = stereo_volume.detach()  # viz: color stereo-depth volume (pre-fusion)
            self._vis_dg = gray_volume.detach()    # stash for visualization
            if self.geo_fixed_w is not None:
                w = torch.full((stereo_volume.shape[0], 1, *stereo_volume.shape[2:]),
                               float(self.geo_fixed_w), device=stereo_volume.device, dtype=stereo_volume.dtype)
            else:
                w = torch.sigmoid(self.geo_reliability(torch.cat([stereo_volume, gray_volume], dim=1)))
            self._vis_gw = w.detach()   # stash for visualization
            stereo_volume = (1.0 - w) * stereo_volume + w * gray_volume
