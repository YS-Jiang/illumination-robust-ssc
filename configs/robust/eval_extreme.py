data_root = 'data/semantickitti_degraded/extreme'
ann_file = 'data/semantickitti/labels'
stereo_depth_root = 'data/semantickitti_degraded/extreme/depth'
camera_used = ['left']
...
train_pipeline = [
    dict(type='LoadMultiViewImageFromFiles', data_config=data_config, load_stereo_depth=True, load_gray=True, load_gray_stereo_depth=True,
         is_train=True, color_jitter=(0.4, 0.4, 0.4)),
    dict(type='SyntheticIlluminationDegradation', prob=0.5, degrade_depth=True),
    ...
    dict(type='CollectData', keys=['img_inputs', 'gt_occ'], 
            meta_keys=['pc_range', 'occ_size', 'raw_img', 'stereo_depth', 'focal_length', 'baseline', 'img_shape', 'gt_depths', 'gray_stereo_depth']),
]
...
test_pipeline = [
    ...
    dict(type='CollectData', keys=['img_inputs', 'gt_occ'],  
            meta_keys=['pc_range', 'occ_size', 'sequence', 'frame_id', 'raw_img', 'stereo_depth', 'focal_length', 'baseline', 'img_shape', 'gt_depths', 'gray_stereo_depth'])
]
...
model = dict(
    ...
    use_rcf=True,
    img_backbone=dict(
        type='DepthAnythingV2Backbone',
        out_channels=640, out_stride=8, input_hw=(384, 1280), freeze_vit=True,
    ),
    img_neck=None,
    depth_net=dict(
        ...
        cross_transformer=dict(
           ...
           encoder=dict(
               ...
               transformerlayers=dict(
                   ...
                   attn_cfgs=[
                       dict(
                           type='DeformCrossAttention_DFA3D', use_rcf=True, rcf_fixed_w=0.5,
                           pc_range=point_cloud_range,
                           num_cams=_num_cams_,
